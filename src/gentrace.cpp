//version modified pintrace bap 8.0
// auteur: lionel.Prat9@gmail.com
// add: taint all
// add: print taint source detected (SYSCALL, ENV, ARGV, ...)
// add: trace function pass (interne & externe) 
//      BUG on host ... verify if valid img
// add: trace instruction where used tainted and print (TAINTED with source taint use) 
//      trace is operationnel in bap, juste print if tainted detected, and possible add if: MV,CMP, ... type instruction (if CMP print REG ou string comp)
// add print in file log or redirect stdout
// optimise print pour acceleration
#include "pin.H"
...
KNOB<bool> InsTaints(KNOB_MODE_WRITEONCE, "pintool",
                     "taintins", "false",
                     "Print tainted instruction");
//InsTaints.Value()
struct FrameBuf {
    ADDRINT addr;
    uint32_t tid;
...
VOID AppendBuffer(std::string insDis,
                  ADDRINT addr,
                  THREADID tid,
                  CONTEXT *ctx,
                  BOOL isBranch,
                  ...
                  )
{
    ...
    if ((log_all || has_taint) && log_addr) {
         ...
         * instructions */
        string addrtaint = tracker->hasTaintInfo(ti->delta);
       // cerr << "FUNC:" << courant_func_name << ") -> at:(" << std::hex << addr << std::dec << ") - in img: " << path_func << endl;
        cerr << "TAINTED INSTRUCTION: " << insDis << " on " << addrtaint << endl;
        if (firstLogged) {
...
VOID InstrTrace(TRACE trace, VOID *v)
{
    ...
    //check debut start prog
    /* Decide if we want to log this trace by examining the entrance address. */
    ADDRINT addr = TRACE_Address(trace);
    PIN_LockClient();
    IMG itmp = IMG_FindByAddress(addr); 
    PIN_UnlockClient();
    if (IMG_Valid(itmp)) {
     courant_func_name = IMG_Name(itmp) + ":" + RTN_FindNameByAddress (addr);
     path_func = StripPath(IMG_Name(itmp).c_str());
    } else {
     courant_func_name = RTN_FindNameByAddress (addr);
     path_func = "";
    }
    if(courant_func_name.find(":main") != string::npos) {
      // debut trace
      start_trace=true;
      tracker->trackFile("start");
    }
    if(start_trace){     
     cerr << "FUNC:" << courant_func_name << ") -> at:(" << std::hex << addr << std::dec << ") - in img: " << path_func << endl;
     if (dontLog(addr)) {
     } else if(InsTaints.Value()) {
...
VOID SyscallEntry(THREADID tid, CONTEXT *ctx, SYSCALL_STANDARD std, VOID *v)
{
    ...
    if (tracker->taintPreSC(si.sf.mutable_syscall_frame()->number(), (const uint64_t *) (si.sf.syscall_frame().argument_list().elem().data()), si.state, start_trace)) {
