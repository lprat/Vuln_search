...
// version patched by lionel.prat9@gmail.com
//
...
GenericTaintChecker::TaintPropagationRule::getTaintPropagationRule(
  ...
  TaintPropagationRule Rule = llvm::StringSwitch<TaintPropagationRule>(Name)
    ...
    .Case("fread", TaintPropagationRule(InvalidArgIndex, 0, true))
    .Case("recv", TaintPropagationRule(InvalidArgIndex, 1, true))
    .Case("recvmsg", TaintPropagationRule(InvalidArgIndex, 1, true))
    .Case("recvfrom", TaintPropagationRule(InvalidArgIndex, 1, true))
...
bool GenericTaintChecker::isStdin(const Expr *E, CheckerContext &C) {
   if (const DeclRefExpr *D = dyn_cast<DeclRefExpr>(E->IgnoreParenCasts())){
    StringRef stdin_namex = D->getDecl()->getName();
    StringRef stdin_name2 = stdin_namex.lower();
    if ((stdin_name2.find("stdin") != StringRef::npos)){

            return true;
     }
  } else {
   SourceLocation DomLoc = E->getExprLoc();
   StringRef DomName = C.getMacroNameOrSpelling(DomLoc);
   StringRef stdin_name2 = DomName.lower();
   std::string stdin_string = stdin_name2.str();
   if ((stdin_string.find("stdin_fileno") != std::string::npos) || (stdin_string.find("0") != std::string::npos)){
             return true;
    }
  }
  ProgramStateRef State = C.getState();
