...
bool TaintTracker::taintPreSC(uint32_t callno, const uint64_t *args, /* out */ uint32_t &state, bool start_trace)
{
  ...
  char filename[128];
  if(!start_trace) return reading_tainted;
  cerr << "Syscall no: " << callno << endl << "Args:" ;
...
size_t tot;
  ...
  switch (state) {
      ...
      case __NR_recvmsg:
        fd = args[0];
        tot = (size_t) bytes;
        msg=(struct msghdr *)(uint32_t *)args[1];
        //if (not((long)bytes <= 0)){
	if((msg->msg_iovlen > 0) && (static_cast <unsigned int> (0) < bytes) && (static_cast <unsigned int> (4294967295) != bytes)){ // nombre de tableau
          if (msg->msg_control != NULL) {
            printf("ADD RECVMSG-CONTROLE %d\n",msg->msg_controllen);
            introMemTaint((ADDRINT)(msg->msg_control), msg->msg_controllen, "recvmsg-control", -1);
          }
          for (size_t iz = 0; iz < msg->msg_iovlen && tot > 0; iz++) {
            iov = &msg->msg_iov[iz];
	    /* get the length of the iovec */
	    iov_tot = (tot > (size_t)iov->iov_len) ?
		(size_t)iov->iov_len : tot;
	    /* taint-source */	
            printf("ADD RECVMSG-IOV %d\n",iov_tot);
            introMemTaint((ADDRINT)iov->iov_base, iov_tot, "recvmsg-iov", 1);
            tot -= iov_tot;
          }
         }
        break;
...
//add by lio
string TaintTracker::hasTaintInfo(context &delta)
{
  string info="";
  cerr << hex ;    
  for (uint32_t i = 0 ; i < count ; i++) {
    if (isReg(values[i].type)) {
      if (getRegTaint(delta, values[i].loc) != NOTAINT) {
        info = "Tainted Reg: " + REG_StringShort((REG)values[i].loc);
       return info;
      }
    } else if (isValid(values[i].type)) {
      if (getTaint(memory,values[i].loc) != NOTAINT) {
	std::stringstream streamt;
	streamt << std::hex <<values[i].loc;
        info="Tainted memory: 0x"+streamt.str();
        return info;
      }
    }  
  }  
